...
(** This is an example of transporting a Hoare-like property of partial 
    correctness over a contract morphism.

    What we mean by this is that, since a contract morphism 
            f : ContractMorphism C C'
    relates successful calls and initializations of C to those of C',
    if there is a Hoare property of partial correctness of C that we wish to 
    prove, and we know it of C', we may be able to prove it about C by 
    observing the correspondence betwween successful calls/initializations 
    of C to those of C'.

    Indeed, this is the case. We show it with an example of a structured
    pool contract.
*)
...
(* Sorry for the ugly tactic definition, but the way this is defined
    didn't export well. *)
