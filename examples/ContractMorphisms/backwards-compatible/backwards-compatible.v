...
(** This is an example of proving that a contract is backwards compatible with
    another contract, e.g. if one contract is an upgraded version of another 
    contract.

    To prove such a fact, we show an *embedding* of contracts C into C', 
    where C is the contract (e.g. an older version) with which C' is meant to 
    be backwards compatible.

    The morphism not only indicates that C' is backwards compatible with C, 
    via the embedding, but also it indicates precisely what functionality of C'
