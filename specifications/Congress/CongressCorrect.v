...
From PhD.Specifications.Congress Require Import Congress.
...
Section Theories.
  ...
  Theorem congress_correct bstate caddr (trace : ChainTrace empty_state bstate) :
    (* congress at caddr in bstate *)
    env_contracts bstate caddr = Some (Congress.contract : WeakContract) ->
    (* exists cstate and inc_calls such that *)
    exists (cstate : Congress.State) (inc_calls : list (ContractCallInfo Congress.Msg)),
      (* cstate is the state of the contract AND *)
      contract_state bstate caddr = Some cstate /\
      (* inc_calls is the list of incoming calls AND *)
      incoming_calls Msg trace caddr = Some inc_calls /\
      (* num cacts in state + 
        length outgoing txns of trace + 
        length outgoing acts <= 
        num_acts created in proposals *)
